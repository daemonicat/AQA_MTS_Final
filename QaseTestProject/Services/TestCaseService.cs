...
public class TestCaseService : ITestCaseService, IDisposable
{
    ...
    {
        var request = new RestRequest("/v1/case/{code}", Method.Post).AddJsonBody(testCase).AddUrlSegment("code", code);
    ...
    {
        var request = new RestRequest("/v1/case/{code}/{id}").AddUrlSegment("code", code).AddUrlSegment("id", id);
        _logger.Info(request);
        return await _client.ExecuteAsync<ApiResult<TestCase>>(request);
    }
    ...
    {
        var request = new RestRequest("/v1/case/{code}/{id}", Method.Post).AddJsonBody(testCase).AddUrlSegment("code", code)
            .AddUrlSegment("id", id);
        _logger.Info(request);
        return await _client.ExecuteAsync<ApiResult<TestCase>>(request);
    }
    ...
    {
        var request = new RestRequest("/v1/case/{id}", Method.Post).AddUrlSegment("code", code).AddUrlSegment("id", id);
        _logger.Info(request);
        return await _client.ExecuteAsync<ApiResult<TestCase>>(request);
    }
